...
- [ ] Add sanity checks at startup to confirm the resolved directories are writable; abort with actionable guidance if not.
- [ ] Modify `aios/utils/async_logging.py` session tracking so it stores `_SESSION_STATE_FILE` under the user cache dir instead of `logs/`, eliminating the WinError 5 spam from the report.
- [ ] Update `aios/gui/components/resources_panel/config_persistence.py` and training workflows to default to ProgramData artifacts paths; expose per-user overrides in settings with validation messages.
...
- [ ] Patch the TkinterWeb help panel wrapper so `safe_post_event` mirrors the original signature (`*args, **kwargs`) and prevents the fetch_styles thread crash.
- [ ] Guard GPU signal handler registration so only the main thread attempts `signal.signal`, eliminating the "signal only works in main thread" warnings even in admin mode.
