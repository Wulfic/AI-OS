...
from .encoding import encode_lines as _encode_lines_helper, adjust_tokenizer_padding as _adjust_tok_padding
...
def train_expert_only(config: "TrainingConfig") -> None:
    ...
    # Save expert (using safetensors format for security and performance)
    expert_dir = Path("artifacts") / "experts" / expert_id
    expert_dir.mkdir(parents=True, exist_ok=True)
    
    expert_path = expert_dir / "expert.safetensors"
    
    try:
        from safetensors.torch import save_file as save_safetensors
        save_safetensors(expert.state_dict(), str(expert_path))
        print({
            "expert_saved": True,
            "format": "safetensors",
        ...
        print({"expert_save_error": "safetensors not installed"})
        raise
