    ...
    *,
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Text encoding failed in featurize_hashing: {e}", exc_info=True)
            bs = b""
        if not bs:
            return v
        ...
        return np.zeros((max(8, int(dim)),), dtype=np.float32)
    ...
    try:
        ...
        for n in ngrams:
            try:
                ...
                logger.warning(f"Invalid n-gram value '{n}', using default 1: {e}")
                n_int = 1
            if n_int <= 0:
                continue
        ...
        if use_char_ngrams:
            try:
                ...
                logger.warning(f"Invalid char_ngram_range '{char_ngram_range}', using default (3, 5): {e}")
                lo, hi = 3, 5
            s = (text.lower() if lowercase else text)
            s = re.sub(r"\s+", " ", s)
