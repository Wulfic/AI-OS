#!/usr/bin/env python3
"""
FULL Dataset Downloader - NO STREAMING
Downloads complete datasets to Z:\training_datasets
All datasets fully downloaded before training
"""

import os
import sys
import json
import time
from pathlib import Path
from typing import List, Dict, Any

print("=" * 100)
print("FULL DATASET DOWNLOADER - NO STREAMING MODE")
print("=" * 100)
print()

# Target directory with 1TB space
OUTPUT_DIR = Path("Z:/training_datasets")
print(f"📁 Target directory: {OUTPUT_DIR}")
print(f"💾 Budget: 1TB (~1000GB)")
print(f"⚠️  ALL DATASETS WILL BE FULLY DOWNLOADED (no streaming)")
print()

# Create output directory
try:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    print("✅ Output directory ready")
except Exception as e:
    print(f"❌ Failed to create output directory: {e}")
    sys.exit(1)

# Check if datasets library is installed
try:
    from datasets import load_dataset, DownloadConfig
    print("✅ HuggingFace datasets library found")
except ImportError:
    print("❌ HuggingFace datasets library not found")
    print("\nInstalling datasets library...")
    import subprocess
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "datasets", "tqdm"])
        from datasets import load_dataset, DownloadConfig
        print("✅ Successfully installed datasets library")
    except Exception as e:
        print(f"❌ Failed to install: {e}")
        sys.exit(1)

print()

# Dataset catalog - OPTIMIZED FOR FULL DOWNLOADS (NO STREAMING)
# Using smaller configs and subsets that can be fully downloaded
DATASET_CATALOG = [
    # === TIER 1: Essential & Quick (71GB) ===
    {
        "name": "WikiText-103",
        "path": "wikitext",
        "config": "wikitext-103-raw-v1",
        "split": "train",
        "size_gb": 0.5,
        "priority": 1,
        "description": "Wikipedia articles - structured text",
        "quality": "⭐⭐⭐⭐⭐"
    },
    {
        "name": "Wikipedia-20220301",
        "path": "legacy-datasets/wikipedia",
        "config": "20220301.en",
        "split": "train",
        "size_gb": 20,
        "priority": 1,
        "description": "Full Wikipedia dump - factual knowledge",
        "quality": "⭐⭐⭐⭐⭐"
    },
    {
        "name": "OpenWebText",
        "path": "Skylion007/openwebtext",
        "config": None,
        "split": "train",
        "size_gb": 38,
        "priority": 1,
        "description": "Reddit-curated web content",
        "quality": "⭐⭐⭐⭐"
    },
    {
        "name": "BookCorpusOpen",
        "path": "bookcorpusopen/bookcorpusopen",
        "config": None,
        "split": "train",
        "size_gb": 6,
        "priority": 1,
        "description": "Books dataset (open version)",
        "quality": "⭐⭐⭐⭐"
    },
    
    # === TIER 2: Educational - FULL DOWNLOAD VERSIONS (150GB) ===
    {
        "name": "FineWeb-Edu-10BT",
        "path": "HuggingFaceFW/fineweb-edu",
        "config": "sample-10BT",
        "split": "train",
        "size_gb": 30,
        "priority": 2,
        "description": "Educational web content - 10B tokens sample (FULL DOWNLOAD)",
        "quality": "⭐⭐⭐⭐⭐"
    },
    {
        "name": "FineWeb-Edu-100BT",
        "path": "HuggingFaceFW/fineweb-edu",
        "config": "sample-100BT",
        "split": "train",
        "size_gb": 100,
        "priority": 2,
        "description": "Educational web content - 100B tokens sample (FULL DOWNLOAD)",
        "quality": "⭐⭐⭐⭐⭐"
    },
    {
        "name": "FineWeb-Edu-350BT",
        "path": "HuggingFaceFW/fineweb-edu",
        "config": "sample-350BT",
        "split": "train",
        "size_gb": 150,
        "priority": 2,
        "description": "Educational web content - 350B tokens sample (FULL DOWNLOAD)",
        "quality": "⭐⭐⭐⭐⭐"
    },
    {
        "name": "C4-en-validation",
        "path": "allenai/c4",
        "config": "en",
        "split": "validation",
        "size_gb": 1,
        "priority": 2,
        "description": "C4 validation set (small, high quality)",
        "quality": "⭐⭐⭐⭐"
    },
    
    # === TIER 3: Large Scale Diverse (300GB) ===
    {
        "name": "RedPajama-V2-Sample",
        "path": "togethercomputer/RedPajama-Data-V2",
        "config": "sample",
        "split": "train",
        "size_gb": 150,
        "priority": 3,
        "description": "RedPajama V2 sample - diverse high quality",
        "quality": "⭐⭐⭐⭐"
    },
    {
        "name": "Dolma-Sample",
        "path": "allenai/dolma",
        "config": "v1_6-sample",
        "split": "train",
        "size_gb": 100,
        "priority": 3,
        "description": "Dolma dataset sample - diverse corpus",
        "quality": "⭐⭐⭐⭐"
    },
    {
        "name": "Pile-Uncopyrighted",
        "path": "monology/pile-uncopyrighted",
        "config": None,
        "split": "train",
        "size_gb": 100,
        "priority": 3,
        "description": "The Pile (uncopyrighted subset)",
        "quality": "⭐⭐⭐⭐"
    },
]

# Calculate total size
total_size = sum(ds["size_gb"] for ds in DATASET_CATALOG)
print("=" * 100)
print("DATASET CATALOG - FULL DOWNLOAD VERSIONS")
print("=" * 100)
print(f"📊 Total datasets: {len(DATASET_CATALOG)}")
print(f"💾 Total size: ~{total_size:.1f} GB")
print()

# Display catalog by priority
for priority in [1, 2, 3]:
    tier_datasets = [ds for ds in DATASET_CATALOG if ds["priority"] == priority]
    tier_size = sum(ds["size_gb"] for ds in tier_datasets)
    tier_name = {
        1: "TIER 1: Essential (Quick)",
        2: "TIER 2: Educational (Full Download)",
        3: "TIER 3: Large-Scale Diverse"
    }[priority]
    
    print(f"\n{tier_name} (~{tier_size:.1f} GB)")
    print("-" * 100)
    for ds in tier_datasets:
        print(f"  • {ds['name']} ({ds['size_gb']}GB) {ds['quality']}")
        print(f"    {ds['description']}")
        print()

print("=" * 100)

# User selection
print("\n📋 DOWNLOAD OPTIONS:")
print("1. Quick Test (65GB) - Tier 1 only")
print("2. Recommended + Educational (346GB) - Tier 1 + 2")
print("3. Full Collection (746GB) - All tiers")
print("4. Custom selection")
print("5. List only (no download)")
print()

choice = input("Select option [1-5] (default: 2): ").strip() or "2"

if choice == "5":
    print("\n✅ Dataset catalog displayed. No downloads performed.")
    sys.exit(0)

# Determine which datasets to download
if choice == "1":
    selected = [ds for ds in DATASET_CATALOG if ds["priority"] == 1]
elif choice == "2":
    selected = [ds for ds in DATASET_CATALOG if ds["priority"] <= 2]
elif choice == "3":
    selected = DATASET_CATALOG
elif choice == "4":
    print("\nCustom selection:")
    selected = []
    for i, ds in enumerate(DATASET_CATALOG, 1):
        answer = input(f"  Download {ds['name']} ({ds['size_gb']}GB)? [y/N]: ").strip().lower()
        if answer == 'y':
            selected.append(ds)
else:
    print("Invalid choice, using Recommended")
    selected = [ds for ds in DATASET_CATALOG if ds["priority"] <= 2]

if not selected:
    print("❌ No datasets selected")
    sys.exit(0)

selected_size = sum(ds["size_gb"] for ds in selected)
print(f"\n✅ Selected {len(selected)} datasets (~{selected_size:.1f} GB)")
print()

# Confirm download
print("⚠️  WARNING: This will download large amounts of data!")
print(f"   Total size: ~{selected_size:.1f} GB")
print(f"   Destination: {OUTPUT_DIR}")
print(f"   Mode: FULL DOWNLOAD (no streaming)")
print()
confirm = input("Proceed with download? [y/N]: ").strip().lower()
if confirm != 'y':
    print("❌ Download cancelled")
    sys.exit(0)

print()
print("=" * 100)
print("STARTING FULL DOWNLOAD (NO STREAMING)")
print("=" * 100)
print()

# Download configuration for optimization
download_config = DownloadConfig(
    cache_dir=str(OUTPUT_DIR / ".cache"),
    resume_download=True,
    max_retries=5,
    num_proc=4
)

# Statistics
stats = {
    "total": len(selected),
    "completed": 0,
    "skipped": 0,
    "failed": 0,
    "start_time": time.time()
}

def download_dataset(ds_info: Dict[str, Any], index: int) -> Dict[str, Any]:
    """Download a single dataset - FULL DOWNLOAD ONLY"""
    result = {
        "name": ds_info["name"],
        "success": False,
        "skipped": False,
        "error": None,
        "samples": 0,
        "time": 0
    }
    
    output_path = OUTPUT_DIR / ds_info["name"].lower().replace("-", "_").replace(" ", "_")
    
    # Check if already downloaded
    if output_path.exists():
        try:
            from datasets import load_from_disk
            existing = load_from_disk(str(output_path))
            result["samples"] = len(existing)
            result["success"] = True
            result["skipped"] = True
            print(f"[{index}/{stats['total']}] ✅ {ds_info['name']} - Already downloaded ({result['samples']:,} samples)")
            return result
        except Exception:
            print(f"[{index}/{stats['total']}] ⚠️  {ds_info['name']} - Existing download corrupted, re-downloading...")
            try:
                import shutil
                shutil.rmtree(output_path)
            except:
                pass
    
    print(f"[{index}/{stats['total']}] 📥 {ds_info['name']} ({ds_info['size_gb']}GB)")
    print(f"   Path: {ds_info['path']}")
    if ds_info.get("config"):
        print(f"   Config: {ds_info['config']}")
    print(f"   Mode: FULL DOWNLOAD (all data to disk)")
    
    start = time.time()
    
    try:
        # FORCE FULL DOWNLOAD - NO STREAMING
        print(f"   📥 Downloading complete dataset...")
        
        if ds_info.get("config"):
            dataset = load_dataset(
                ds_info["path"],
                ds_info["config"],
                split=ds_info["split"],
                download_config=download_config,
                trust_remote_code=False  # Disable trust_remote_code warning
            )
        else:
            dataset = load_dataset(
                ds_info["path"],
                split=ds_info["split"],
                download_config=download_config,
                trust_remote_code=False
            )
        
        print(f"   💾 Saving {len(dataset):,} samples to disk...")
        dataset.save_to_disk(str(output_path))
        
        result["success"] = True
        result["samples"] = len(dataset)
        result["time"] = time.time() - start
        
        print(f"[{index}/{stats['total']}] ✅ {ds_info['name']} - Complete!")
        print(f"   Samples: {result['samples']:,}")
        print(f"   Time: {result['time']/60:.1f} minutes")
        print(f"   Location: {output_path}")
        
    except Exception as e:
        result["error"] = str(e)
        result["time"] = time.time() - start
        print(f"[{index}/{stats['total']}] ❌ {ds_info['name']} - FAILED")
        print(f"   Error: {e}")
    
    print()
    return result

# Download datasets sequentially
print("🚀 Starting full downloads (sequential for reliability)...")
print()

results = []
for i, ds in enumerate(selected, 1):
    result = download_dataset(ds, i)
    results.append(result)
    
    # Update stats
    if result["success"]:
        if result["skipped"]:
            stats["skipped"] += 1
        else:
            stats["completed"] += 1
    else:
        stats["failed"] += 1
    
    # Show progress
    elapsed = time.time() - stats["start_time"]
    remaining = len(selected) - i
    if stats["completed"] > 0:
        avg_time = elapsed / (stats["completed"] + stats["skipped"])
        eta = avg_time * remaining / 60
        print(f"📊 Progress: {i}/{len(selected)} | Completed: {stats['completed']} | Skipped: {stats['skipped']} | Failed: {stats['failed']}")
        print(f"⏱️  Elapsed: {elapsed/60:.1f}m | ETA: {eta:.1f}m")
        print("-" * 100)
        print()

# Final summary
total_time = time.time() - stats["start_time"]

print()
print("=" * 100)
print("DOWNLOAD SUMMARY")
print("=" * 100)
print(f"✅ Completed: {stats['completed']}/{stats['total']}")
print(f"⏭️  Skipped (already downloaded): {stats['skipped']}/{stats['total']}")
print(f"❌ Failed: {stats['failed']}/{stats['total']}")
print(f"⏱️  Total time: {total_time/60:.1f} minutes ({total_time/3600:.2f} hours)")
print()

if stats["completed"] > 0 or stats["skipped"] > 0:
    print("📚 DOWNLOADED DATASETS (FULLY ON DISK):")
    total_samples = 0
    for result in results:
        if result["success"]:
            status = "✅" if not result["skipped"] else "⏭️ "
            samples = result['samples']
            total_samples += samples
            print(f"   {status} {result['name']}: {samples:,} samples")
    print(f"\n   📊 TOTAL: {total_samples:,} samples across {stats['completed'] + stats['skipped']} datasets")
    print()
    
    print("🔗 TRAINING CONFIGURATION:")
    print(f"   Dataset root: {OUTPUT_DIR}")
    print()
    print("   All datasets are FULLY DOWNLOADED and ready to use!")
    print("   No streaming - all data is on Z: drive")
    print()
    print("🚀 NEXT STEPS:")
    print("1. Configure HRM Training Panel:")
    print(f"   Dataset path: Z:/training_datasets/<dataset_name>")
    print("2. DISABLE 'Teacher-as-dataset' checkbox")
    print("3. Start training immediately (no download delays!)")
    print()
    print("📋 Example paths:")
    for result in results:
        if result["success"]:
            ds_name = result['name'].lower().replace("-", "_").replace(" ", "_")
            print(f"   - Z:/training_datasets/{ds_name}")
    print()

if stats["failed"] > 0:
    print("⚠️  FAILED DOWNLOADS:")
    for result in results:
        if not result["success"]:
            print(f"   ❌ {result['name']}: {result['error']}")
    print()
    print("💡 TROUBLESHOOTING:")
    print("   - Check internet connection")
    print("   - Ensure Z: drive has enough space")
    print("   - Try downloading failed datasets individually")
    print("   - Some datasets may require authentication or have been moved")
    print()

print("=" * 100)
print(f"🎉 DOWNLOAD PROCESS COMPLETE!")
print(f"📁 All datasets FULLY downloaded to: {OUTPUT_DIR}")
print(f"⚡ Ready for immediate training (no streaming delays)")
print("=" * 100)
