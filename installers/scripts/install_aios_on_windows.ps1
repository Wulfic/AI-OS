...
function Install-Aios() {
  ...
  # Install FlashAttention-2 for extreme context support (Windows: precompiled wheel required)
  if ($Gpu -eq 'cuda' -or ($Gpu -eq 'auto' -and $gpuInfo.HasNvidia)) {
    Write-Host "[i] Installing FlashAttention-2 (precompiled wheel for Windows)..."
    try {
      # Detect versions for wheel naming: flash_attn-2.7.4+cu{CUDA}torch{TORCH}cxx11abiFALSE-cp{PY}-cp{PY}-win_amd64.whl
      $pyVer = & $pythonPath -c "import sys; print(f'{sys.version_info.major}{sys.version_info.minor}')"
      $torchInfo = & $pythonPath -c "import torch; import re; tv=torch.__version__.split('+')[0]; cv=torch.version.cuda; print(f'{tv}|{cv}')"
      $parts = $torchInfo.Split('|')
      $torchVer = $parts[0]
      $cudaVer = $parts[1] -replace '\.', ''
      
      # Construct wheel URL from HuggingFace lldacing repo (validated source)
      ...
      Write-Host "[i] Installing FlashAttention from wheel..."
